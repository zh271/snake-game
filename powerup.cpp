...
void PowerUpManager::apply(GameState& state, Snake& snake, PowerUpType type) {
    switch (type) {
        case POWER_BIG_FOOD: applyBigFood(state, snake); break;
        case POWER_MAGNET:   applyMagnet(snake);         break;
        case POWER_DRILL:    applyDrill(snake);          break;
        case POWER_DIGEST:   applyDigest(snake);         break;
        case POWER_SLOW:     applySlow(snake);           break;
        default: break;
    }
}
